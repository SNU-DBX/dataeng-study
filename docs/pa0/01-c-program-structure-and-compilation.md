...
이 중에 힙과 스택은 프로그램 실행 중 필요에 따라 크기가 변하며, 일반적으로 서로를 향하는 방향으로 확장됩니다.
실제 방향과 배치는 시스템 및 컴파일러에 따라 달라질 수 있습니다.
...
- **`#include <header>`**: 표준 라이브러리 또는 사용자가 정의한 헤더 파일
...
### 기타 요소
그 외에도 소스 파일은 다양한 요소들을 가질 수 있습니다. 아래 예시 파일을 참고해주세요.

<details>
<summary>예제: <code>source_file.c</code></summary>
...
<details>
<summary>예제: <code>local_header.h</code></summary>
...
사람이 읽을 수 있는 형태로 작성된 소스 코드를 컴퓨터가 실행할 수 있는 형태로 변환하는 과정이 필요합니다. 우리는 이 과정을 **빌드 (Build)** 라고 부릅니다.  
